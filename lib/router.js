...
// 返回Node.js模块里面所有http方法(get, post, options ...)
...
methods.forEach(function (method) {
  Router.prototype[method] = function (name, path, middleware) {
    ...
    // path是字符串表示有三个参数: name, path, middleware
    if (typeof path === 'string' || path instanceof RegExp) {
      middleware = Array.prototype.slice.call(arguments, 2);
    } else {
      // 表示只有两个参数(path, middleware)
      ...
      while (matched.route && i--) {
        ...
        while (ii--) {
          if (layer.stack[ii].constructor.name === 'GeneratorFunction') {
            // 从第一个开始链接生成generator链
            // router.get('/a', function* a1(next){}, function* a2(next){}, function* a3(next){})
            // a1 => a2 => a3 ...
            next = layer.stack[ii].call(this, next);
          } else {
            // 否则将next作为参数传进函数, 并将函数转成promise执行, layer确保layer.stack里面的都必须是函数
  ...
  return function *allowedMethods(next) {
    ...
    if (!this.status || this.status === 404) {
      ...
      if (!~implemented.indexOf(this.method)) {
        // 抛出未实现方法的异常, 检查用户是否设置了options.throw
        if (options.throw) {
...
Router.prototype.register = function (path, methods, middleware, opts) {
  ...
  // 生成的route全部保存到stack中,
  // 通过/分割长度相同的path对应的路由, 被保存在数组中邻近的位置
 ...
 * @private
 */
// 根据path, method(get, post ...)匹配到指定layer
Router.prototype.match = function (path, method) {
  var layers = this.stack;
  var layer;
  var matched = {
    path: [], // 只匹配到了path
    pathAndMethod: [], // 匹配到了path 和 method(get, post ...)
    route: false
  };
